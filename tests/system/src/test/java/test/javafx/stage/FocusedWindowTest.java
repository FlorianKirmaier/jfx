...
public class FocusedWindowTest {
    ...
    public void testLeakOnce() throws Exception {
        ...
        Platform.runLater(() -> {
            ...
            //    Stage tmp = new Stage();
            //    tmp.setTitle("tmp stage");
            //    tmp.setScene(new Scene(new TextField()));
            //    tmp.show();
                Platform.runLater(() -> {
                    System.out.println("Closing closedFocusedStage");
                    closedFocusedStage.close();
                    Platform.runLater(() -> {
                        System.out.println("RequestFocus closedFocusedStage");
                        closedFocusedStage.requestFocus();
                        System.out.println("RequestFocus textfield");
                        textField.requestFocus();
                        Platform.runLater(() -> {
                        //    System.out.println("Closing TMP");
                        //    tmp.close();
                            leakLatch.countDown();
                        });
                    });
        ...
        Assert.assertTrue("Timeout waiting for FX runtime to start", leakLatch.await(15, TimeUnit.SECONDS));
